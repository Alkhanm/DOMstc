        ...
        <v-container>
          <v-row>
            <v-col cols="12" sm="5" md="5">
              <v-text-field color="white" label="Código" v-model="product.code" required type="number"
                hint="Código de barras do produto" />
            </v-col>
            <v-col cols="12" sm="7" md="7">
              <v-text-field label="Descrição" v-model="product.description" hint="Titulo/nome do produto">
              </v-text-field>
            </v-col>
            <v-col cols="12" sm="5" md="5">
...
async function onClickSave() {
  ...
  try {
    ...
    alert.type = "SUCCESS";
    alert.msg = `O produto: ${product.value.description} salva com sucesso`
  } catch {
    alert.type = "ERROR";
